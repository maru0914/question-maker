...
const currentResults = ref(
    JSON.parse(localStorage.getItem(`books.${props.question.book.id}.results`)),
);

const isSuccess = ref(
    currentResults.value
        ? currentResults.value.find(
              (result) => result.question_id === props.question.id,
          )?.is_success
        : null,
);

const showAnswer = ref(
    isSuccess.value !== null && isSuccess.value !== undefined,
);
...
const showSaveButton = computed(() => {
    ...
    const isFinished =
        currentResults.value &&
        currentResults.value.length === props.questions_count;
...
const setResult = (item) => {
    if (currentResults.value) {
        currentResults.value = currentResults.value.filter(
            (result) => result.question_id !== item.question_id,
        );
        currentResults.value.push(item);
        localStorage.setItem(
            `books.${props.question.book.id}.results`,
            JSON.stringify(currentResults.value),
        );
    } else {
...
const saveResult = () => {
    router.post(route('books.challenges.store', props.question.book.id), {
...
<template>
    <section>
        <div>
            <h2 class="text-xl font-bold">問題文</h2>
            <Panel class="mt-3">
                {{ question.body }}
            </Panel>
        </div>
        <div class="mt-4">
            <div class="text-center">
                <BigButton v-if="!showAnswer" @click="showAnswer = !showAnswer"
                    >答えを見る</BigButton
                >
            </div>
            ...
            <Transition>
                <div v-if="showAnswer">
                    <h2 class="mr-5 text-xl font-bold">答え</h2>

                    <Panel class="mt-3 whitespace-pre-line">
                        {{ question.answer }}
                    </Panel>
                </div>
            </Transition>
    ...
    <section
        v-if="showAnswer && !showSaveButton"
    ...
    <section
        v-if="showAnswer && showSaveButton && $page.props.auth.user"
    ...
    >
        ...
        <BigButton class="text-xl" @click="saveResult"
            >結果を保存する</BigButton
        >
    </section>
    ...
    >
        ...
        <BigButton
            class="text-xl"
            @click="router.get(route('books.show', question.book.id))"
            >問題集を終了する</BigButton
        >
    </section>
</template>
